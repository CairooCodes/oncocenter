...
if (isset($_GET['edit_id']) && !empty($_GET['edit_id'])) {
  $id = $_GET['edit_id'];
  $stmt_edit = $DB_con->prepare('SELECT * FROM agreements WHERE id =:uid');
...
if (isset($_POST['btnsave'])) {
  ...
  if (empty($name)) {
    $errMSG = "Por favor, insira o nome do convênio";
  }
  if ($imgFile) {
  ...
  if (!isset($errMSG)) {
    $stmt = $DB_con->prepare('UPDATE agreements
    SET 
    name=:uname,
    link=:ulink,
    img=:upic
  ...
  <div class="ml-auto mb-6 lg:w-[75%] xl:w-[80%] 2xl:w-[85%]">
    ...
    <div class="px-6 pt-6 2xl:container">
      <form action="" method="POST" enctype="multipart/form-data">
        <div class="space-y-6">
          <input value="<?php echo $name ?>" name="name" class="w-full text-sm px-4 py-3 focus:bg-gray-100 border border-gray-300 rounded-none focus:outline-none focus:border-color1" type="" placeholder="Nome do convênio">
          <input value="<?php echo $link ?>" name="link" class="w-full text-sm px-4 py-3 focus:bg-gray-100 border border-gray-300 rounded-none focus:outline-none focus:border-color1" type="" placeholder="Link de redirecionamento">
          <div class="items-center lg:grid lg:grid-cols-2">
            <div class="flex justify-center">
            <img class="h-48 w-48" src="./uploads/agreements/<?php echo $img; ?>" onerror="this.src='../assets/img/semperfil.png'" alt="Profile">
            </div>
                ...
                <div class="m-4">
                  ...
                  <div class="flex items-center justify-center w-full">
                    <label class="flex flex-col w-full h-40 border-4 border-dashed hover:bg-gray-100 hover:border-gray-300">
                      ...
                      <img id="preview" class="absolute inset-0 w-full h-42">
